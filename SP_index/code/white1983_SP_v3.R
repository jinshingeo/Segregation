...
#
# ============================================================
# 파일명 : white1983_SP_v3.R
# 목적   : White(1983) 공간 인접성 지수(SP) 패키지 — 사용자 친화 버전
# 변경점 : v2 대비
#   - 데모 데이터 로드 방식 수정: 하드코딩 절대 경로 → file.choose()
#     (교수님 원본 코드와 동일한 방식 / 어느 컴퓨터에서도 실행 가능)
# 변경점 : v1 대비
...
#          실행 중 파일 탐색기가 열리면 segdata.rda 파일을 선택
# ============================================================
...
# ---- 데이터 로드 ----
# [file.choose()]
#   - 파일 탐색기 팝업을 열어 사용자가 직접 파일을 선택하게 함
#   - 교수님 원본 코드(load(file.choose()))와 동일한 방식
#   - 어느 컴퓨터에서도 실행 가능 (하드코딩 경로 없음)
# [load()]
#   - .rda 파일 안에 저장된 R 객체를 현재 환경으로 불러옴
#   - segdata 라는 이름의 data.frame이 로드됨
load(file.choose())   # 실행 시 파일 탐색기 팝업 → segdata.rda 선택
